---
name: android-epic
description: Plan and execute medium-to-hard Android EPICs in user-authorized slices with resumable checkpoints. Use for EPIC planning or slice execution, not routine Android fixes.
---

# Android EPIC

Apply this workflow to the user's Android EPIC. Respect explicit user overrides of these workflow preferences. Do not load the human guide for execution. The user supplies desired behavior and constraints; you discover technical details, maintain records, and generate concrete continuation prompts.
...
Provide a ready-to-paste continuation prompt suited to the recommended conversation. For the same conversation, use a short authorization or resume message with the actual slice ID; do not require repeating the skill name or known paths. For a new conversation, invoke $android-epic and include actual absolute paths to the specification and progress log, plus the actual slice ID. If skill discovery is unavailable, include the actual path to this SKILL.md instead. For partial work, resume the current slice; for completed work, propose the next slice for authorization. Fill all engineering fields yourself; do not return placeholders for the user to construct.
